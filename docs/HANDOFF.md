...
| D4 | deploy. Architect decisions (owner delegated): deploy from **`main`** — `redesign/gitit-os` was a clean fast-forward of origin/main (33 commits) and main was ff-merged locally 2026-08-21; repo `gitit1/gitit-portfolio` is **PUBLIC** → Coolify "public repository" app, **no deploy key needed** (one owner action fewer). Still hers: (1) the word to **push main**; (2) **`ANTHROPIC_API_KEY`** entered in Coolify UI (app → Environment Variables) — or handed to the architect to set via API. Then architect: Coolify project `gititregev-site` + app (build_pack=dockerfile, is_static=false, port 3000, domains `https://gititregev.com`,`https://www.gititregev.com`, env `ALLOWED_ORIGIN=https://gititregev.com`), deploy via API, watch logs | architect + owner | ready — waiting for push word + key |
